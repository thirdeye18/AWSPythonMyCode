...
def commandpush(devicecmd): # devicecmd==dict
    ...
    for ip in devicecmd.keys(): # looping through the dict
        print ('Handshaking. .. ... connecting with {ip}') # fstring
...
def devicereboot(ipAddys):
    for ip in ipAddys:
        print(f"Connecting to {ip}...EBOOTING NOW!")
...
def main():
    ...
    print (f"{crayons.green('Welcome to the network device command pusher')}") # welcome message
