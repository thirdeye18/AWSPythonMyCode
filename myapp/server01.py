...
@app.route('/delrec', methods = ['DELETE'])
def delrec():
    try:
        nm = request.form['nm']     # get name to delete

        #connect to db
        with sql.connect("database.db") as con:
            cur = con.cursor()
            sql = f"DELETE FROM students WHERE name=?"
            cur.execute(sql, (nm,))
            con.commit()
            msg = "record sucessfully removed"
    except:
        msg = "error removing record"
    finally:
        return render_template("resultdel.html", msg = msg)
